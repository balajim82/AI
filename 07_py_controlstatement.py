"""
  1. Indentation: In Python, indentation is mandatory and defines code blocks, replacing the curly braces used in many other programming languages.
     All statements within the same block (such as an if statement, for loop, or function body) must have the same level of indentation.
     In Java usally when we write any program class, Function, Loops,if..
     while start with { and end with } bracket.
    In Python we used to ":" after any type of statements like above. It treats as parent and child
     Ex: if x>5 :
            print("Value is True")
         else:
...
Control statements are used to control the flow of execution in a program. They allow you to make decisions, repeat actions, and manage the flow of your code based on certain conditions.
 ...
 3. enumarate : this improtent techniq in python and used in for loop to get index and value of the list.
"""
...
# enumarate
list_values = ["balaji", "raji", "suri", "rithvik"]
for i in enumerate(list_values, start=1):
    print(i)
