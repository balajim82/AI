...
9. Same Like List comprehesnion here we have dict comprehension.
    syntax:  {key_expression: value_expression for item in iterable if condition}
10. deepcopy, in python it's one of the library. Once install deepcopy then import and can use it.
   The purpose of deepcopy is that, if copy any list then it wont refee any object of original list.
   both list will be indepdent.
   Ex: list1=[1,2,3,4]
       list2=list1 --> list1 and list2 both will be same reference pointing.
   from copy import deepcopy
      org_list=[1,2,3,4,5]
      deepcpylist=deepcopy(org_list)
      so here org_list and deepcpylist not refer same object.

11. using fromkeys() function:  The **dict.fromkeys() **method in Python is a class method used to create a new dictionary from an iterable of keys, assigning a single, shared value to all of them. If no value is specified, None is used as the default value.
     keys_iterable (required): An iterable object (e.g., list, tuple, set, string) whose elements will be used as the keys in the new dictionary.
     value (optional): The value that will be assigned to every key. If omitted, each key's value will be None.
    Important Consideration: Mutable Values
     Be careful when using a mutable object (like a list or a dictionary) as the value argument.
     All keys will share a reference to the same mutable object in memory.
     Modifying the value through one key will affect all other keys.
...
# dict comprehension
z = {i: i**2 for i in range(1, 11) if i % 2 == 0}
print(z)
...
keys = ["k1", "k2"]
# The list [1] is created once and shared
shared_list = [1]
d = dict.fromkeys(keys, shared_list)

print(f"Before update: {d}")

# Modifying the list through one key affects all keys
d["k1"].append(2)

print(f"After update: {d}")
# Output: Before update: {'k1': [1], 'k2': [1]}
# Output: After update: {'k1': [1, 2], 'k2': [1, 2]}
